...
public class SearchByState implements Searchable {
    @Autowired
    ...
    public List<People> search(String name) {
        List<People> peoplesSelected = peopleService.getPeoples(session);
    ...
    public int numberOfStatePeople(String name){
        List<People> peopleList=search(name);
        return(peopleList==null?0:peopleList.size());
    }

    public double percentageOfStatePeople(String name){
        List<People>people = peopleService.getPeoples(session);
        List<People>statePeople = search(name);
        double peopleNumber=(people==null?0:people.size());
        double statePeopleNumber= (statePeople==null?0:statePeople.size());
        return  (statePeopleNumber*100)/peopleNumber;
    }
