...
@Service("searchByAgeHandicap")
public class SearchByAgeHandicap extends AlgorithmsHandicap implements SearchableHandicap {
    @Override
    public List<HandicapPeople> search(String name,HttpSession session) {
        int age = Integer.parseInt(name);
        List<HandicapPeople> peoplesSelected = handicapPeopleService.displayHandicapPeople(session);
