...
<html>
  <head>
    ...
    <title>Localnici handicap</title>
...
<div>
     ...
     <div>
     <a href="<c:url value="/listaHandicap/${settlement.name}"/> " class="btn btn-light btn-block">${settlement.name}</a>
     </div>
     </c:forEach>
     </div>
</div>
...
<div>
          <table id="people" class="table table-striped" width='100%' border="1px solid black">
           ...
           <th>Varsta</th>
           <th>Observatii</th>
           <th>Tipul handicapului </th>
           <th>Optiuni</th>
           </tr>
           <c:forEach items="${handicap}" var="people">
            <tr ${people.state eq "nesalvat" ? 'style="background-color:red"' : 'style="background-color:green"'}>
              ...
              <td><c:out value = "${people.type}"/></td>
              <td><a href="<c:url value="/listaHandicap/${people.id}/detalii"/> " class="btn btn-info">Detalii/Modifica</a></td>
              </tr>
              </c:forEach>
              </table>
              </div>
