...
<div>
<a href="<c:url value="/localnici/${settlement.name}"/> " class="btn btn-light btn-block">${settlement.name}</a>
</div>
</c:forEach>
</div>
</div>
...
<table id="people" class="table table-striped" width='100%' border="1px solid black">
           ...
           <c:forEach items="${peopleList}" var="people">
            <tr ${people.state eq "nesalvat" ? 'style="background-color:red"' : 'style="background-color:green"'}>
              ...
              <td><a href="<c:url value="/localnic/${people.id}/detalii"/> " class="btn btn-info">Detalii/Modifica</a></td>
          </td>
              </tr>
              </c:forEach>
