...
<div class  = "col-sm-2">
		    ...
		    <p>Selecteaza localitatea:</p>
		         <c:forEach items = "${Settlements}" var ="settlement">
		            <div>
				         <a href="<c:url value="/localniciIzolati/${settlement.name}"/> " class="btn btn-light btn-block">${settlement.name}</a>
		            </div>
		          </c:forEach>
	        </div>
</div>
...
<div class= "col-sm-10">
			<div>
				<table id="people" class="table table-striped" width='100%' border="1px solid black">
					...
					<c:forEach items="${isolated}" var="people">
